<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>Insert title here</title>
</head>
<body>
		<%@include file="header.jsp" %>
		<!-- 가격 안내 -->
		<div class="col-md-12 text-center py-5">
			<p><Strong><h4>taxicompay_data_system</h4></Strong>가격 안내<Strong class="must_do">*</Strong>
			<div class="col-md-12">
					
				<div class="col-md-12">
					월 30,000원 선택 시 표 시스템 제공 됩니다.
					<div class="col-md-12">
					<img class="img-fluid" src="/taxicompany_data_system/img/graph.PNG">
					</div>	
				</div>
				
				<div class="col-md-12 text-center py-5">
					월 60,000원 선택 시 차트 시스템까지 제공 됩니다.
					<div class="col-md-12 row ">
						<div class="col-md-6 px-5 py-5">
							<img class="img-fluid" src="/taxicompany_data_system/img/chart1.PNG">
						</div>
						<div class="col-md-6 py-5">
							<img class="img-fluid" src="/taxicompany_data_system/img/chart2.PNG">
						</div>
					</div>
				</div>
				
				<div class="col-md-12 text-center py-5">
					월 90,000원 선택 시 운행 데이터 시스템까지 제공 됩니다.
					<div class="col-md-12 row ">
						<div class="col-md-6 py-5">
							<img class="img-fluid" src="/taxicompany_data_system/img/datainfo.png">
						</div>
						<div class="col-md-6 py-5">
							<img class="img-fluid" src="/taxicompany_data_system/img/datainfo2.png">
						</div>
					</div>
				</div>
			</div>
		</div>
		<%@include file ="footer.jsp" %>
</body>
</html>