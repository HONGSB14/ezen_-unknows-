...
<body>
	...
	<div class="container">
		<h3> 게시물 상세 </h3>
		<% 
			int bno = Integer.parseInt( request.getParameter("bno") );		// boardlist 넘어온 게시물번호 값 받아오기
		...
		%>
			...
			<a href="boardupdate.jsp?bno=<%=board.getBno()%>"><button>수정</button> </a>		<!--작성자와 로그인된 id가 동일하면 보이는 버튼  -->
		<%
			} 
		%>
		...
		<table>
			...
			%> 
				<tr> <td>첨부파일<td> <td>첨부파일이 존재하지 않습니다.</td> </tr> <!-- 첨부파일 다운로드 -->
			<%	}else{ // 첨부파일이 있을경우 %>
